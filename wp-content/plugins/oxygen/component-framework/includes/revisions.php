...
class Oxygen_Revisions {
    ...
    static function create_revision( $post_id ) {
        ...
        if( metadata_exists( "post", $post_id, "ct_builder_shortcodes" ) ){
            // First copy the current shortcode three as a new revision
            $current_three = get_post_meta( $post_id, "ct_builder_shortcodes", true);
            ...
            $inserted_revision = add_post_meta( $post_id, "ct_builder_shortcodes_revisions", $current_three );
            add_post_meta( $post_id, "ct_builder_shortcodes_revisions_dates", time() );

            // Delete the older revisions to only keep:
            // - The 20 most recent versions
            // – Then, 24 versions, one for each hour for the preceding day, since the oldest revision
            // – Then, 30 versions, one for each day for the preceding month, since the oldest revision
            $revisions = self::get_post_meta_db( $post_id, "ct_builder_shortcodes_revisions" );
            ...
            if( is_array( $revisions ) && count( $revisions ) > $max_recent_revisions ) {
	            ...
	            $last_date_for_hourly_revisions = $max_recent_revisions - 1;
	            $last_date_for_daily_revisions = NULL;
	            $hourly_revisions = 0;
	            $daily_revisions = 0;
	            ...
	            for( ; $index <= count( $revisions ); $index++ ){
                    ...
                    if( intval( $dates[ $last_date_for_hourly_revisions ]->meta_value)  - intval( $dates[ $index ]->meta_value ) < 3600 ) {
	                    ...
	                    $last_date_for_hourly_revisions = $index;
	                    $hourly_revisions++;
                    }
                    if( $hourly_revisions >= 24 ) {
    ...
    static function restore_revision( $post_id, $revision_id ) {
        ...
        update_post_meta( $post_id, 'ct_builder_shortcodes', $found->meta_value );
